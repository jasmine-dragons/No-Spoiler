...
train_labels = train['spoiler'].to_numpy().astype(np.int32)

test_text = (test['page'].map(str) + ' ~~~ ' + test['sentence'].map(str)).to_numpy()
test_labels = test['spoiler'].to_numpy().astype(np.int32)
...
model.save('model_tvtropes')

model = load_model('model_tvtropes')
